...
def rangesumasu(df):
    ...
    table_df = reason_df.drop(columns=['ASUNodes','ASU upgrade success rate','ASUusagerate']).reset_index(drop=True)
    table_df = (table_df.query("NonASUNodes != 0")).reset_index(drop=True)
    #st.table(table_df)
    # st.markdown("""---""")
    st.subheader("Non-ASU By City And Type")

    a = non_df.query("NonASUNodes != 0")
    print(a)
    reasonfig = px.scatter(non_df.query("NonASUNodes != 0"), x="Months", y="NonASUNodes", size="NonASUNodes", color="City",
                           hover_name="City", size_max=60, title='Non-ASU By City')
...
def checkbycu(df):
    ...
    scatter_chart = px.scatter(df, x='City', y='ASUusagerate', color='ASUusagerate', title='ASU Rate')
...
def checkbycuzti(df):
    ...
    fig = px.pie(df_pie, values='Qty', names='Category',title='ToT')
...
if __name__ == '__main__':
    st.set_page_config(page_title='The summary of ENO program in MLC Beta',layout="wide")
    st.header('2022 Data Summary of ENO MLC Beta ')
